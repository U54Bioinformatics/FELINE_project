...
## NEW section gets mutation status
mutat_dd<-data.table(fread("/Users/jason/Downloads/FELINE_gene_cnv_barplot.patient.STable_allCNV.three_IDs.txt") %>% dplyr::select(Patient.Study.ID,variable,value))[variable%in%c("CDK6","ESR1","AKT3","TP53" )]
setnames( mutat_dd ,old=c("variable","value"),new=c("gene_dna","mutationtype"))
mutat_dd$mutationtype%>%unique
mutat_dd$gene_dna%>%unique
mutat_dd[, mutation_status:="Normal"]
mutat_dd[mutationtype!="N", mutation_status:="Mutated"]
...
onegene_dna <- mutat_dd[ gene_dna==wch_gene]
require(merTools)
...
path_clin_longCNA<- merge(path_clin_long,onegene_dna,by="Patient.Study.ID")
#path_clin_longCNA[mutationtype=="loss",mutation_status:="Normal"]
#path_clin_longCNA[ARM=="B",mutation_status:="Normal"]
path_clin_longCNA[,Treat:="Letrozole alone"]
path_clin_longCNA[ARM=="B",Treat:="Intermittent high \n dose ribociclib"]
path_clin_longCNA[ARM=="C",Treat:="Continuous low \n dose ribociclib"]
path_clin_longCNA$Treat <- factor(path_clin_longCNA$Treat, levels = c("Letrozole alone", "Intermittent high \n dose ribociclib", "Continuous low \n dose ribociclib"))

ggplot(path_clin_longCNA,aes(x=Day,y=ssGSEA)) + geom_violin(aes(group=interaction(Day,mutation_status)))+facet_wrap(mutation_status~ARM)
ggplot(path_clin_longCNA[Day==0],aes(x=mutation_status,y=ssGSEA,fill=mutation_status)) + geom_violin(aes(group=interaction(Day,mutation_status)))+facet_wrap(~Treat)+theme_classic(base_size=18)+
  scale_x_discrete(name="ERS1 copy number",labels=c("Loss","Normal"))+theme(legend.position="none",aspect.ratio=1)+ylab(llll)
ggplot(path_clin_longCNA[Day==0],aes(x=mutation_status,y=ssGSEA,fill=mutation_status)) + geom_violin(aes(group=interaction(Day,mutation_status)))+facet_wrap(~Treat)+theme_classic(base_size=18)+
  scale_x_discrete(name="TP53 copy number",labels=c("Loss","Normal"))+theme(legend.position="none",aspect.ratio=1)+ylab(llll)
ggplot(path_clin_longCNA[Day==0],aes(x=mutation_status,y=ssGSEA,fill=mutation_status)) + geom_violin(aes(group=interaction(Day,mutation_status)))+facet_wrap(~Treat)+theme_classic(base_size=18)+
  scale_x_discrete(name="AKT3 copy number",labels=c("Gain","Normal"))+theme(legend.position="none",aspect.ratio=1)+ylab(llll)
ggplot(path_clin_longCNA[Day==0],aes(x=mutation_status,y=ssGSEA,fill=mutation_status)) + geom_violin(aes(group=interaction(Day,mutation_status)))+facet_wrap(~Treat)+theme_classic(base_size=18)+
  scale_x_discrete(name="CDK6 copy number",labels=c("Gain","Normal"))+theme(legend.position="none",aspect.ratio=1)+ylab(llll)

ggplot(path_clin_longCNA[Day==180],aes(x=mutation_status,y=ssGSEA,fill=mutation_status)) + geom_violin(aes(group=interaction(Day,mutation_status)))+facet_wrap(~Treat)+theme_classic(base_size=18)+
  scale_x_discrete(name="CDK6 copy number",labels=c("Gain","Normal"))+theme(legend.position="none",aspect.ratio=1)+ylab(llll)


NEWXDAT2 <-rbind( data.table( unique(path_clin_longCNA%>%dplyr::select(Patient.Study.ID,ARM,mutation_status)),day_fact="0"),
                  data.table( unique(path_clin_longCNA%>%dplyr::select(Patient.Study.ID,ARM,mutation_status)),day_fact="14"),
                  data.table( unique(path_clin_longCNA%>%dplyr::select(Patient.Study.ID,ARM,mutation_status)),day_fact="180"))
NEWXDAT2$Patient.Study.ID <-as.factor(NEWXDAT2$Patient.Study.ID)

Mod1 <- lmer(ssGSEA ~ mutation_status*day_fact*ARM + (1+day_fact|Patient.Study.ID), REML=FALSE,data= path_clin_longCNA) 
summary(Mod1)
...
PRED.lme4<-data.table( data.table(cbind(PRED.lme4,cinfreg)) %>% group_by(ARM,day_fact,mutation_status)%>%dplyr::mutate(lwr=min(fit),upr=max(fit),fit=mean(fit) ))
PRED.lme4<-data.table(PRED.lme4 %>% group_by(ARM,day_fact,mutation_status)%>%dplyr::mutate(lwr=min(fit),upr=max(fit),fit=mean(fit) ))
...
PRED.lme4[, y_init:=sum(is_day0*yi), by=c("Patient.Study.ID","ARM","mutation_status")]
PRED.lme4[, mu_init:=sum(is_day0*y), by=c("Patient.Study.ID","ARM","mutation_status")]

PRED.lme4[Patient.Study.ID=="001-101"]
PRED.lme4[, upr1:=quantile(yi-y_init,probs =0.75), by=c("day_fact","ARM","mutation_status")]
PRED.lme4[, lwr1:=quantile(yi-y_init,probs =0.25), by=c("day_fact","ARM","mutation_status")]
...
PRED.lme4[, mu_i_init:=sum(is_day0*yi), by=c("Patient.Study.ID","ARM","mutation_status","pathwayscore")]
ggplot(PRED.lme4,
       aes(y=y-mu_init,x=dum_day,group=mutation_status,col=mutation_status))+
  geom_line(aes(group=interaction(pathwayscore,mutation_status,ARM)),size=2,alpha=1)+#geom_line(aes(y=yi-y_init,group=interaction(pathwayscore,dynamic_class3,Patient.Study.ID)),size=1,alpha=1)+
  geom_ribbon(aes(ymax=upr1,ymin=lwr1,fill=mutation_status),alpha=0.4,col=NA)+
  facet_grid(.~Treat,scales="free") + theme_classic(base_size=20)+
  scale_color_discrete(name="TP53 copy number",labels=c("Loss","Normal"))+
  scale_fill_discrete(name="TP53 copy number",labels=c("Loss","Normal"))+
  scale_x_continuous(name="Day",breaks=0:2,labels=c(0,14,180))+
  ylab(paste0(llll[II])) + theme(aspect.ratio=1)+
...
ggplot(PRED.lme4,
  ...
  geom_ribbon(aes(ymax=upr1,ymin=lwr1,fill=mutation_status),alpha=0.4,col=NA)+
  facet_grid(.~Treat,scales="free") + theme_classic(base_size=20)+
  scale_color_discrete(name="AKT3 copy number",labels=c("Gain","Normal"))+
  scale_fill_discrete(name="AKT3 copy number",labels=c("Gain","Normal"))+
  scale_x_continuous(name="Day",breaks=0:2,labels=c(0,14,180))+
  ylab(paste0(llll[II])) + theme(aspect.ratio=1)+
  geom_line(aes(y=yi-mu_i_init,x=dum_day,group=Patient.Study.ID),linetype=2)+ theme(aspect.ratio=1)
...
ggplot(PRED.lme4,
  ...
  geom_ribbon(aes(ymax=upr1,ymin=lwr1,fill=mutation_status),alpha=0.4,col=NA)+
  facet_grid(.~Treat,scales="free") + theme_classic(base_size=20)+
  scale_color_discrete(name="ERS1 copy number",labels=c("Loss","Normal"))+
  scale_fill_discrete(name="ERS1 copy number",labels=c("Loss","Normal"))+
  scale_x_continuous(name="Day",breaks=0:2,labels=c(0,14,180))+
  ylab(paste0(llll[II])) + theme(aspect.ratio=1)+
  geom_line(aes(y=yi-mu_i_init,x=dum_day,group=Patient.Study.ID),linetype=2)+ theme(aspect.ratio=1)
...
ggplot(PRED.lme4,
  ...
  geom_ribbon(aes(ymax=upr1,ymin=lwr1,fill=mutation_status),alpha=0.4,col=NA)+
  facet_grid(.~Treat,scales="free") + theme_classic(base_size=20)+
  scale_color_discrete(name="CDK6 copy number",labels=c("Gain","Normal"))+
  scale_fill_discrete(name="CDK6 copy number",labels=c("Gain","Normal"))+
  scale_x_continuous(name="Day",breaks=0:2,labels=c(0,14,180))+
  ylab(paste0(llll[II])) + theme(aspect.ratio=1)+
  geom_line(aes(y=yi-mu_i_init,x=dum_day,group=Patient.Study.ID),linetype=2)+ theme(aspect.ratio=1)
