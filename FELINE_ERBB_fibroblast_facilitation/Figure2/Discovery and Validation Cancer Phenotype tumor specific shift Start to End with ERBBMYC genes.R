...
discData<-data.table(read.csv(file="/Users/jason/Jason Griffiths Dropbox/jason griffiths/Cancer_pheno_evo/data/FELINE2/Cancer GF analyses/SourceData/Figure2/DiscoveryCohortDEGPrePost.csv"))
geneList<- as.character(c("EGFR","ERBB2","ERBB3","ERBB4","MYC"))
...
cpm_i1 <- data.table( fread( paste0(CPMlocs1, CPMfiles1[grep("Cancer",CPMfiles1)]) ) )[V1%in%geneList]   # load full gene expression
...
cpm_i2<- data.table( fread( paste0(CPMlocs2, CPMfiles2[grep("Cancer",CPMfiles2)]) ) )[V1%in%geneList]   # load full gene expression
...
allDEadditional<-allDE
save(allDEadditional,  file="/Users/jason/Dropbox/Cancer_pheno_evo/data/FELINE2/Gene expression landscapes/cancer gene expression landscape/Validation Additional Tumor specific shift strt vs end cancer gene expression scrna.RData")
...
load( file="/Users/jason/Dropbox/Cancer_pheno_evo/data/FELINE2/Gene expression landscapes/cancer gene expression landscape/Validation Tumor specific shift strt vs end cancer gene expression scrna.RData")
#allDE 
pltthis1 <- allDE[rn!="(Intercept)"][  ] 
pltthisAdded <- allDEadditional[rn!="(Intercept)"][  ] 
pltthisAdded$gene <- factor(pltthisAdded$gene , levels=c("EGFR","ERBB2","ERBB3","ERBB4","MYC"))
pltthisAdded[,dir:="up"]


valData <- data.table( pltthisAdded%>%dplyr::select(-c(dynamic_class) ))
...
ggplot( valData, aes(y=Estimate, x=gene) ) + theme_classic(base_size=26) + geom_point()+
  ...
  labs(x= "Gene" , y= "Fold change in cancer cell expression") +
  facet_wrap(~Treatment)+
  geom_boxplot(aes(fill=dir,group=gene)) +coord_flip() +
...
pltthisAdded%>%dplyr::select(-c(dynamic_class) )
valDataAdded <- data.table( pltthisAdded%>%dplyr::select(-c(dynamic_class) ))
setnames( valDataAdded , old=c("rn"), new=c("ModelParamLM"))
write.csv(valDataAdded,file="/Users/jason/Jason Griffiths Dropbox/jason griffiths/Cancer_pheno_evo/data/FELINE2/Cancer GF analyses/SourceData/ValidationCohortDEGaddedERBBPrePost.csv")
...
DEGdiscDataAdded <- data.table(read.csv(file="/Users/jason/Jason Griffiths Dropbox/jason griffiths/Cancer_pheno_evo/data/FELINE2/Cancer GF analyses/SourceData/Figure2/DiscoveryCohortDEGaddedERBBPrePost.csv"))
valDataAdded <- data.table( read.csv(file="/Users/jason/Jason Griffiths Dropbox/jason griffiths/Cancer_pheno_evo/data/FELINE2/Cancer GF analyses/SourceData/Figure2/ValidationCohortDEGaddedERBBPrePost.csv") )


d_out<-rbind( data.table(cohort="Discovery",DEGdiscData%>%dplyr::select(-X) ),
              data.table(cohort="Discovery",DEGdiscDataAdded%>%dplyr::select(-X) ),
              data.table(cohort="Validation",valData %>%dplyr::select(-X) ),
              data.table(cohort="Validation",valDataAdded %>%dplyr::select(-X)  ))

d_out$gene <- factor(d_out$gene, levels=c(consistentgenesUDwn$gene,c("EGFR","ERBB2","ERBB3","ERBB4", "MYC")))
d_out[Treatment=="letrozole + ribo",Treatment:="letrozole + ribociclib"]  
...
top15 <- c( gsub("gene","",top15),c("EGFR","ERBB2","ERBB3","ERBB4", "MYC"))
plotdd<-d_out[gene%in%top15]
plotdd$gene<-factor(plotdd$gene,levels=rev(top15))
plotdd<-plotdd[order(gene)]
plotdd[,catag:="detected"]
plotdd[gene%in%c("EGFR","ERBB2","ERBB3","ERBB4", "MYC"),catag:="verified"]
...
#ggsave(   file="/Users/jason/Dropbox/Cancer_pheno_evo/images and presentations/FELINE 2/Cancer GF analyses/Discovery and Validation Single cell most significant changes in cancer cell expression in resistant cells YRCohortERBBMYCadded.pdf", height=10, width = 10)
...
#ggsave(   file="/Users/jason/Dropbox/Cancer_pheno_evo/images and presentations/FELINE 2/Cancer GF analyses/Discovery and Validation Single cell most significant changes in cancer cell expression in resistant cells YRCohortERBBMYCaddedLong.pdf", height=10, width = 10)
