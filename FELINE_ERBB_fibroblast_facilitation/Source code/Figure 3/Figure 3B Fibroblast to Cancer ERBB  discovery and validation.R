rm(list=ls())
library(glmnet);require(dplyr);require(data.table);require(tidyr);require(ggplot2); require(lmerTest);require(ggsci)
# load(file ="/Users/jgriffiths/Jason Griffiths Dropbox/jason griffiths/Cancer_pheno_evo/data/FELINE2/Communication output merged ALL ARMS/PopulationCommunicationMerged ALL ARMS.RData" )#CCI,allphenotypes, uu,perIndiv,
...
# DiscoveryFibroCancerComms <- CCI[ReceptorPhenoCelltype=="Cancer cells"][LigandPhenoCelltype=="Fibroblasts"]
...
#write.csv(DiscoveryFibroCancerComms, file= paste0(Intermediateloc,"SourceData_Figure_3B_DiscoveryFibrotoCancerComms.csv"))
DiscoveryFibroCancerComms <- data.table(read.csv( file= paste0(Intermediateloc,"SourceData_Figure_3B_DiscoveryFibrotoCancerComms.csv")))




wchComms<- unique( c( "EGFR","ERBB2","ERBB3","ERBB4"))
isApath<-sapply( 1:length(wchComms),function(ii){
  grepl(paste0("_",wchComms[ii]),unique(DiscoveryFibroCancerComms$Pair.Name) ) 
  } ) 
l1 <- unique(DiscoveryFibroCancerComms$Pair.Name)[rowSums(isApath)>0 ]

# select start and end timepoint data for fibroblast cancer interactions
GFFibroCancerComm <- DiscoveryFibroCancerComms[Pair.Name%in%l1][
  Day!=14
]#|LigandPhenoCelltype=="Cancer cells"]
...
#ggsave(   file="/Users/jason/Dropbox/Cancer_pheno_evo/images and presentations/FELINE 2/Cancer GF analyses/Fibroblast to Cancer Growth ERBB Factor communication receiver increase relative to baselineYR.png", height=10, width = 12)
...
#load(file ="/Users/jgriffiths/Jason Griffiths Dropbox/jason griffiths/Cancer_pheno_evo/data/FELINE2/cohort2 Communication output merged/PopulationCommunicationMerged ALL ARMS cohort2.RData" )#CCI,allphenotypes, uu,perIndiv,
#CCI[,Treat:="CombinationRibo"]
#CCI[ARM=="A",Treat:="LetrozoleAlone"]
#CCI[,scaleTransduction:=scale(TransductionMu,center=F),by=c("Pair.Name")] #"ReceptorPhenoCelltype",
#CCI[!is.finite(TransductionMu),scaleTransduction:=0]
...
# ValidationFibroCancerComms <- CCI[ReceptorPhenoCelltype=="Cancer cells"][LigandPhenoCelltype=="Fibroblasts"]
# Load discovery communication data
#write.csv(ValidationFibroCancerComms, file= paste0(Intermediateloc,"SourceData_Figure_3B_ValidationFibrotoCancerComms.csv"))
ValidationFibroCancerComms <- data.table(read.csv( file= paste0(Intermediateloc,"SourceData_Figure_3B_ValidationFibrotoCancerComms.csv")))
...
# select start and end timepoint data for fibroblast cancer interactions
GFFibroCancerComm <- ValidationFibroCancerComms[Pair.Name%in%l1][  Day!=14]
GFFibroCancerComm[,npatSamp:=length(unique(Patient.Study.ID)),by=Pair.Name]
GFFibroCancerComm[,propPatSamp:=npatSamp/length(unique(GFFibroCancerComm$Patient.Study.ID))]
GFFibroCancerComm[,npairSamp:=length(unique(Pair.Name)),by=Patient.Study.ID]
GFFibroCancerComm[,propPairSamp:=npairSamp/length(unique(GFFibroCancerComm$Pair.Name))]
...
ggplot(GRPPLOT2, 
  ...
  scale_fill_manual(name="Timepoint",values=c("yellow","red")) +
  labs(x="Cancer growth factor receptor", y="Fibroblast-cancer cell communication \n (relative to baseline average)")+ theme(aspect.ratio=1,legend.position="bottom")
#ggsave(   file="/Users/jason/Dropbox/Cancer_pheno_evo/images and presentations/FELINE 2/Cancer GF analyses/Validation Fibroblast to Cancer Growth ERBB Factor communication receiver increase relative to baselineYR.pdf", height=10, width = 12)
