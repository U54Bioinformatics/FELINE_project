...
# Analyze the steady state of the system as cancer facilitation by fibroblasts (beta_C) is varied
...
# Explore effect of dose on steady state
resX <- rbindlist(lapply(seq(0,3,length=40), function(i){
  par_tt<-par_vals
  par_tt["X"]<-i
  return(data.table(X=i,
                    t(steady(y= inits, func=ode_fun, parms= par_tt,time=c(0,Inf),method="runsteady")$y )))
}))
ggplot(resX, aes(x=X,y=C,col=M)) + geom_path() +theme_classic()

# Explore effects of varying both dose and facilitation effects on cancer steady state
