rm(list=ls())   
require(data.table); require(dplyr); require(ggplot2); require(tidyr);require(igraph);require(ggsci)
...
TotCCImrg <- merge(TotCCI,uu,by=c("key_")) 
...
#save(TotCCImrg, SumTotComm0,file="~/Dropbox/Cancer_pheno_evo/data/FELINE2/TotalCommunicationAllArms/TotalCommunicationAllArms.RData")

load(file="~/Dropbox/Cancer_pheno_evo/data/FELINE2/TotalCommunicationAllArms/TotalCommunicationAllArms.RData")
...
## Preliminary analyses below assessing total communication from across the entie tme
assessment <- rbindlist(lapply(1:nrow(lu_lm), function(i){
  ...
  if(length(unique(yi[Day==max(Day)]$dynamic_class3))>1  & length(unique(yi[Day!=max(Day)]$dynamic_class3))>1  &
    ...
    m2 <- lm(log(1+scaleTransduction) ~ log(1+Day) , data=yi[dynamic_class3=="Response"])      
    m3 <- lm(log(1+scaleTransduction) ~ dynamic_class3 , data=yi[Day==180])      
    m4 <- lm(log(1+scaleTransduction) ~ dynamic_class3 , data=yi[Day==0])         
...
save(assessment,print.this,responseRelated,responseRelated4,file="~/Dropbox/Cancer_pheno_evo/data/FELINE2/When cells communicate_communication changes AllArms/Total signal differences and trends by response mods.RData")
write.csv(print.this,file="~/Dropbox/Cancer_pheno_evo/data/FELINE2/Trends in communication AllArms/Trends in TOTAL communication by response AllArms.csv")

load(file="~/Dropbox/Cancer_pheno_evo/data/FELINE2/When cells communicate_communication changes AllArms/Total signal differences and trends by response mods.RData")


# Make plots of some of the top communication differences between resistant and sensitive tumors over time
# first under ribo treatment
Treatplot<-"CombinationRibo"
for(CC in unique(responseRelated4$PhenoCelltype) ){
  ...
  if(nrow(plotdd[Treat==Treatplot])>0){
    ggplot(plotdd[Treat==Treatplot],#[grepl("IL",Pair.Name)],
      ...
      ggsave(filename=paste0("~/Dropbox/Cancer_pheno_evo/images and presentations/FELINE 2/LR TOTAL trends AllArms/RiboCombination Increasing and decreasing total communication",CC,".png"),width=10,height=10)
  }
}

# second under letrozole treatment
...
for(CC in unique(responseRelated4$PhenoCelltype) ){
  ...
  if(nrow(plotdd[Treat==Treatplot])>0){
    ...
    ggsave(filename=paste0("~/Dropbox/Cancer_pheno_evo/images and presentations/FELINE 2/LR TOTAL trends AllArms/LetrozoleAlone Increasing and decreasing total communication",CC,".png"),width=8,height=8)
  }
}

# look at initial differences
...
# plot these communications with big intiial differences between resistant and sensitive tumors  
# first under ribo treatment
for(CC in unique(InitDiffsRibo$PhenoCelltype) ){
  ...
  if(nrow(plotdd)>0){
    ...
    ggsave(filename=paste0("~/Dropbox/Cancer_pheno_evo/images and presentations/FELINE 2/LR TOTAL initial condition diffs AllArms/RibociclibCombination Initial differences in total communication",CC,".png"),width=8,height=8)
  }
}
# second under letrozole treatment
for(CC in unique(InitDiffsLetr$PhenoCelltype) ){
  ...
  if(nrow(plotdd)>0){
    ...
    ggsave(filename=paste0("~/Dropbox/Cancer_pheno_evo/images and presentations/FELINE 2/LR TOTAL initial condition diffs AllArms/Letrozolealone Initial differences in total communication",CC,".png"),width=8,height=8)
  }
}
...
ggplot(plotdd,  aes(y=log(1+scaleTransduction),x=log(1+Day),col=dynamic_class3))+geom_point()+geom_smooth(method="lm")+
...
ggsave(filename=paste0("~/Dropbox/Cancer_pheno_evo/images and presentations/FELINE 2/LR TOTAL initial condition diffs AllArms/RibociclibCombination Initial differences in total communication.png"),width=16,height=8)
...
ggsave(filename=paste0("~/Dropbox/Cancer_pheno_evo/images and presentations/FELINE 2/LR TOTAL initial condition diffs AllArms/Letrozolealone  Initial differences in total communication.png"),width=16,height=8)
