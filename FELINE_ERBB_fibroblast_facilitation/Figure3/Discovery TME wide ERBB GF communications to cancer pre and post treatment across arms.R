...
load(file="/Users/jason/Dropbox/Cancer_pheno_evo/data/FELINE2/TotalCommunicationAllArms/TotalCommunicationAllArms.RData")
...
ggplot(GFFibroCancerComm2, 
  ...
  facet_wrap(~Pair.Name,scales="free", ncol=5) +  theme(aspect.ratio=2 ,axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
...
#ggsave(   file="/Users/jason/Dropbox/Cancer_pheno_evo/images and presentations/FELINE 2/Cancer GF analyses/Discovery cohort2 Growth factor signalling to cancer cells increased relative to baseline ERBB only.pdf", height=10, width = 12)
...
ggplot(GRPPLOT2, 
  ...
  labs(x="Cancer growth factor receptor", y="Growth factor signalling \n to cancer cells")+ theme(aspect.ratio=1,legend.position="bottom")
#ggsave(   file="/Users/jason/Dropbox/Cancer_pheno_evo/images and presentations/FELINE 2/Cancer GF analyses/Discovery cohort2 Growth factor signalling to cancer cells increased relative to baseline ERBB onlyYRnewsquare.pdf", height=8, width = 8,dpi=320)
##########ggsave(   file="/Users/jason/Dropbox/Cancer_pheno_evo/images and presentations/FELINE 2/Cancer GF analyses/Discovery cohort2 Growth factor signalling to cancer cells increased relative to baseline ERBB onlyYRnewsquare.pdf", height=7, width = 7,dpi=320)
...
m1 <- lmer( I(scalelnTransductionFC-d0PatMean ) ~TimePoint+ (1|Pair.Name:Patient.Study.ID)+(1|Patient.Study.ID)  , data=GRPPLOT[Receptor=="EGFR"] )
m2 <- lmer( I(scalelnTransductionFC-d0PatMean ) ~TimePoint+ (1|Pair.Name:Patient.Study.ID)+(1|Patient.Study.ID)  , data=GRPPLOT[Receptor=="ERBB2"] )
m3 <- lmer( I(scalelnTransductionFC-d0PatMean ) ~TimePoint+ (1|Pair.Name:Patient.Study.ID)+(1|Patient.Study.ID)  , data=GRPPLOT[Receptor=="ERBB3"] )
m4 <- lmer( I(scalelnTransductionFC-d0PatMean ) ~TimePoint+ (1|Pair.Name:Patient.Study.ID)+(1|Patient.Study.ID) , data=GRPPLOT[Receptor=="ERBB4"] )
m3 <- lmer( scalelnTransductionFC  ~TimePoint+ (1|Pair.Name)   , data=GRPPLOT[Receptor=="ERBB3"] )
summary(m1)
summary(m2)
summary(m3)
summary(m4)

summary( lmer( (scalelnTransductionFC)~TimePoint+(1|Patient.Study.ID),GRPPLOT[Receptor=="ERBB3"][Ligand=="NRG1"] ))
summary( lmer(I(scalelnTransductionFC-d0PatMean )~ TimePoint+(TimePoint|Pair.Name)+ (1|Pair.Name:Patient.Study.ID),GRPPLOT[Receptor=="ERBB3"] ))

ggplot( GRPPLOT[Receptor=="ERBB3"][] , aes(y=I(scalelnTransductionFC-d0PatMean),x=TimePoint))+geom_violin() +geom_point() 

ggplot( GRPPLOT1[Receptor=="ERBB3"][] , aes(y=y,x=TimePoint))+geom_violin() +geom_point() 
summary( lm(y~ -1+ TimePoint,GRPPLOT2[Receptor=="ERBB3"] ))
