...
ggplot(GFFibroCancerComm2,#[grepl("_ERBB",Pair.Name)|grepl("_EGFR",Pair.Name)], 
...
ggplot(GFFibroCancerComm2,#[grepl("_ERBB",Pair.Name)|grepl("_EGFR",Pair.Name)], 
       aes(y=scalelnTransductionFC-d0PatMean, x=TimePoint ))+geom_violin()+theme_classic()+
  facet_wrap(~Pair.Name,scales="free", ncol=5) 
...
GRPPLOT1<- data.table(GRPPLOT %>%group_by(Patient.Study.ID,TimePoint,Receptor)%>% dplyr::summarise(y=mean(scalelnTransductionFC-d0PatMean , na.rm=T)))
GRPPLOT1[,grpx:=paste(Patient.Study.ID,TimePoint,sep="AT")]
GRPPLOT2<- data.table( GRPPLOT1 %>% select(-c(Patient.Study.ID,TimePoint)) %>% spread(grpx,y,fill=0) %>%gather(grpx,y,-Receptor))
...
#ggsave(   file="/Users/jason/Dropbox/Cancer_pheno_evo/images and presentations/FELINE 2/Cancer GF analyses/Fibroblast to Cancer Growth ERBB Factor communication receiver increase relative to baseline.png", height=10, width = 12)
#ggsave(   file="/Users/jason/Dropbox/Cancer_pheno_evo/images and presentations/FELINE 2/Cancer GF analyses/Fibroblast to Cancer Growth ERBB Factor communication receiver increase relative to baseline.pdf", height=10, width = 12)
...
ggplot(GRPPLOT2, 
  ...
  labs(x="Cancer growth factor receptor", y="Fibroblast-cancer cell communication \n (relative to baseline average)")+ theme(aspect.ratio=1,legend.position="bottom")
#ggsave(   file="/Users/jason/Dropbox/Cancer_pheno_evo/images and presentations/FELINE 2/Cancer GF analyses/Fibroblast to Cancer Growth ERBB Factor communication receiver increase relative to baselineYRsquare.pdf", height=8, width = 8,dpi=320)


discData <- data.table( merge(GRPPLOT2 , unique(GRPPLOT%>%dplyr::select(Patient.Study.ID,Treat,ARM)),by="Patient.Study.ID", all.x=T) %>%
                           dplyr::select(c(Receptor,Patient.Study.ID ,ARM,Treat,y,TimePoint ) ))
setnames(discData , old=c("Receptor","y","Treat"), new=c("CommunicationReceptor","logFibroblasttoCancerCommunicationScore","treat"))
discData$TimePoint <- factor(discData$TimePoint, levels=c("Pre treatment","Post treatment"))
#write.csv(discData,file="/Users/jason/Jason Griffiths Dropbox/jason griffiths/Cancer_pheno_evo/data/FELINE2/Cancer GF analyses/SourceData/DiscoveryCohortFibroblastERBBcommunicationwithtoCancerPrePost.csv")
...
ggplot(discData, 
  ...
  scale_fill_manual(name="Timepoint",values=c("yellow","red")) +
  labs(x="Cancer growth factor receptor", y="Fibroblast-cancer cell communication \n (relative to baseline average)")+ theme(aspect.ratio=1,legend.position="bottom")
#ggsave(   file="/Users/jason/Dropbox/Cancer_pheno_evo/images and presentations/FELINE 2/Cancer GF analyses/Fibroblast to Cancer Growth ERBB Factor communication receiver increase relative to baselineYR.pdf", height=10, width = 12)
...
m0 <- lmer( I(scalelnTransductionFC-d0PatMean ) ~ 0+Receptor + TimePoint:Receptor + (1|Pair.Name:Patient.Study.ID)+ (1|Patient.Study.ID) , data= discStatsData[] )
summary(m0)
#write.csv(discStatsData,file="/Users/jason/Jason Griffiths Dropbox/jason griffiths/Cancer_pheno_evo/data/FELINE2/Cancer GF analyses/SourceData/DiscoveryCohort_Stats_FibroblastERBBcommunicationwithtoCancerPrePost.csv")

discStatsData <- data.table(read.csv(file="/Users/jason/Jason Griffiths Dropbox/jason griffiths/Cancer_pheno_evo/data/FELINE2/Cancer GF analyses/SourceData/Figure3/DiscoveryCohort_Stats_FibroblastERBBcommunicationwithtoCancerPrePost.csv"))

discStatsData$Patient.Study.ID%>%unique()
#discData <- data.table(read.csv(file="/Users/jason/Jason Griffiths Dropbox/jason griffiths/Cancer_pheno_evo/data/FELINE2/Cancer GF analyses/SourceData/Figure3/DiscoveryCohortFibroblastERBBcommunicationwithtoCancerPrePost.csv"))
discData$Patient.Study.ID%>%unique()



ggplot(GRPPLOT2, 
       aes(y=Receptor, x=Patient.Study.ID ,fill= y ))+geom_tile()+theme_classic(base_size=36)+
  scale_fill_viridis_c(name="Fibroblast to cancer \n GF communication",option="C") +
  labs(y="Communication receiver", x="Patient tumor")+ theme(legend.position="bottom")+
  facet_wrap(~TimePoint,scales="free_x", ncol=2) +  theme(aspect.ratio=1 ,axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+theme(axis.text.x=element_text(size=14),
                                                                                                                                              strip.text=element_text(size=38))+
  guides(fill=guide_colourbar(barwidth=20))
ggsave(   file="/Users/jason/Dropbox/Cancer_pheno_evo/images and presentations/FELINE 2/Cancer GF analyses/Fibroblast to Cancer Growth Factor communication receiver averaged heatmap increase relative to baseline.png", height=10, width = 12)

ggplot(GRPPLOT2, 
       aes(y=Receptor, x=Patient.Study.ID ,fill= y ))+geom_tile()+theme_classic(base_size=36)+
  scale_fill_viridis_c(name="Fibroblast to cancer \n GF communication",option="C") +
  labs(y="Communication receiver", x="Patient tumor")+ theme(legend.position="bottom")+
  facet_wrap(~TimePoint,scales="free_x", ncol=2) +  theme(aspect.ratio=1.1 ,axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+theme(axis.text.x=element_blank(),                                                                                                                                        strip.text=element_text(size=38))+
  guides(fill=guide_colourbar(barwidth=20))
ggsave(   file="/Users/jason/Dropbox/Cancer_pheno_evo/images and presentations/FELINE 2/Cancer GF analyses/Fibroblast to Cancer Growth Factor communication receiver averaged heatmap increase relative to baselineB.png", height=10, width = 12)


ggplot(GRPPLOT2, 
       aes(y=Receptor, x=Patient.Study.ID ,fill=y ))+geom_tile()+theme_classic(base_size=24)+
  scale_fill_viridis_c(name="Fibroblast to cancer \n GF communication (scaled)",option="magma") +
  labs(y="Communication receiver", x="Patient tumor")+ theme(legend.position="bottom")+
  facet_wrap(~TimePoint,scales="free_x", ncol=2) +  theme(aspect.ratio=1.5 ,axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+theme(axis.text.x=element_text(size=14))
ggsave(   file="/Users/jason/Dropbox/Cancer_pheno_evo/images and presentations/FELINE 2/Cancer GF analyses/Fibroblast to Cancer Growth Factor communication receiver averaged heatmap increase relative to baseline.png", height=10, width = 10)
