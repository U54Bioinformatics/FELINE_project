...
cell_types_all <-c("Cancer_cells","Normal_epithelial_cells")
DAY=c(0,14,180)
ARMS <-c("A","B","C")
...
common_ssgsea <-  intersect(ssgsealist[[1]]$Gene_Set, ssgsealist[[2]]$Gene_Set)
n_ssgsea <- length(common_ssgsea)
ssgsealist[[1]] <- ssgsealist[[1]][Gene_Set %in% common_ssgsea]
ssgsealist[[2]] <- ssgsealist[[2]][Gene_Set %in% common_ssgsea]
SSGSEA <- cbind( ssgsealist[[1]],ssgsealist[[2]][,-1])
...
ss_dd <- t( dim_est_input ) ;


## Perform Intrinsic dimension estimation
dim_estout1 <- est.boxcount(ss_dd , nlevel = 30, cut = c(0.05, 0.95))
...
full_dd_transp <- t( as.matrix(fullcel_gen_sub, rownames= "Gene_Set" ) ) 

#save(upreds_full_dd_transp, upreds_Cell.ID ,umap_data_tt,file= "/Users/jason/Downloads/datasavetst.RData" )
load(file= "/Users/jason/Downloads/datasavetst.RData" )
...
pathwaysNamesUsed <- colnames(ss_dd)
u_dat <- merge(metadd[Day %in% DAY][ARM %in% ARMS],
               data.table( Cell.ID=upreds_Cell.ID, upreds_full_dd_transp, allgene_dd_transp ) ,by="Cell.ID")

methodcor= "spearman"
corVall <-cor(u_dat[]%>%dplyr::select(paste0("V",1:NN)),u_dat[]%>%dplyr::select(pathwaysNamesUsed),method=methodcor)#corV1<-cor(u_dat$V1,full_dd_transp)
Subtype<-metadd[1]$Celltype_subtype
save(umap_data_tt,u_dat,SSGSEA,DAY,cell_types_all,ARMS,common_ssgsea,n_ssgsea,corVall,
     Subtype,pathwaysNamesUsed,
     file=paste0("~/Dropbox/Cancer_pheno_evo/data/FELINE2/PhenotypesAllArms/newjoint",cell_types_all[2],Subtype[1],".RData"))

corResponse_Vs <- cor(log(u_dat[u_dat$V2<5][]$prop_change),u_dat[u_dat$V2<5][]%>%dplyr::select(V1:V3))
methodcor= "spearman"
...
corV3<-cor(u_dat[u_dat$V2<5]$V3,u_dat[u_dat$V2<5]%>%dplyr::select(colnames(ss_dd)),method=methodcor)
corV3ord<-corV3[,order(-abs(corV3))]
corV4<-cor(u_dat[u_dat$V2<5]$V4,u_dat[u_dat$V2<5]%>%dplyr::select(colnames(ss_dd)),method=methodcor)
corV4ord<-corV4[,order(-abs(corV4))]
nn=20
corV1ord[1:nn]
corV2ord[1:nn]
corV3ord[1:nn]
