...
def read_meta_with_cluster(infile, cell_anno):
    ...
    output = re.sub(r'.txt', r'.infercnv_anno.txt', infile)
    ...
    file_df['hpca_main_type'] = col_new_cell_type1
    file_df['Encode_main_type'] = col_new_cell_type2
    file_df['Infercnv_CNA'] = col_new_cell_type3
    
    file_df.to_csv(output, sep="\t", index=False) 


#Cell.ID Infercnv_CNA
#FEL011_M_AAACGAACACAAGTGG       CNA
#FEL011_M_AAACGCTGTTAAGAAC       CNA
def read_infercnv_anno(infile):
    data = defaultdict(lambda : list())
    file_df = pd.read_table(infile, header=0)
    for i in range(file_df.shape[0]):
        data[file_df['Cell.ID'][i]] = [file_df['hpca_main_type'][i], file_df['Encode_main_type'][i], file_df['Infercnv_CNA'][i]]
    return data
...
def main():
    ...
    cell_anno = read_infercnv_anno(args.infercnv)
