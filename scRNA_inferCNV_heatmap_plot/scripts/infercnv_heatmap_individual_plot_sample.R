...
#Annotation file
annot_df <- fread(paste(out_samp, ".cell_metadata.txt", sep=""), header=T)
#names(annot_df) <- c("Cell.ID", "Sample", "nCount_RNA", "Total_Reads_k", "nFeature_RNA", "S", "G2M", "Phase", "percent.mt", "Celltype", "seurat_clusters")
...
infer_name = ""
if (in_type == "final"){
   infer_txt  = paste(out_samp, ".infercnv.observations.RDS", sep="")
...
#CNA_infer <- read.csv(infer_txt, check.name=F, header=T, sep="\t")
CNA_infer <- readRDS(infer_txt)
genes <- CNA_infer$Gene.ID
CNA_infer$Gene.ID <- NULL
#CNA_infer <- as.matrix(CNA_infer)
rownames(CNA_infer) <- genes
out_name  <- infer_name
print(CNA_infer[1:3,1:3])
...
#Random subset
if( out_samp == "FEL011043" | out_samp == "FEL011046"){
    cells <- colnames(CNA_infer)
    cells_randi_id <- sort(sample(1:length(cells), 10000, replace=FALSE))
    cells_rand <- cells[cells_randi_id] 
    CNA_infer <- CNA_infer[, cells_rand] 
}
...
#print dimension
print("Matrix CNA_infer dimension")
print(dim(CNA_infer))
print("Matrix annot_df dimension")
print(dim(annot_df))
...
names_Palette <- colorRampPalette(brewer.pal(8, "Set1"))(length(levels(annot_df$Sample)))
...
##color
#color_ct <- c("#E7298A", "#66A61E", "#006633", "light sea green")
#ct <- c("Cancer cells", "Normal epithelial cells", "Stromal cells", "Immune cells")
color_ct <- c("light sea green", "#006633", "#66A61E", "#E7298A", "gray")
##
#Cell type
celltype_level <- c("Immune cells", "Stromal cells", "Normal epithelial cells", "Cancer cells", "Unclassified")
#annot_df$Celltype <- factor(annot_df$Celltype, levels= unique(annot_df$Celltype))
annot_df$Celltype <- factor(annot_df$Celltype, levels= celltype_level)
#names_Palette <- colorRampPalette(brewer.pal(8, "Dark2"))(length(levels(annot_df$Celltype)))
names_Palette <- color_ct
names(names_Palette) <- levels(annot_df$Celltype)
fontsize = 12
hr4 = rowAnnotation(
  "Cell type" = annot_df$Celltype,
  col= list("Cell type"=names_Palette),
  na_col = "grey", border = TRUE, show_annotation_name = T,
  annotation_legend_param = list("Cell type" = list(title = "Cell type", legend_height = unit(5, "cm"), title_gp = gpar(fontsize = fontsize), labels_gp = gpar(fontsize = fontsize))))
...
print("check point 3")
####No Clustering
#if ( as.numeric(noclust) > 0 ) { # split by sample
if ( FALSE ) {
...
#if ( as.numeric(noclust) > 0 ) { # split by seurat_clusters
if ( FALSE ) {
    print(c("noclust", noclust))
    print('Plot nocluster plot ......')
    ha <- Heatmap(CNA_infer_M, cluster_rows = F, cluster_columns = F, show_row_names = T, show_heatmap_legend = FALSE,
...
if ( as.numeric(noclust) > 0 ) { ## split by celltype
#if ( FALSE ) {
    print(c("noclust", noclust))
    print('Plot nocluster plot ......')
    fontsize = 12
    ha <- Heatmap(CNA_infer_M, cluster_rows = F, cluster_columns = F, show_row_names = T, show_heatmap_legend = FALSE,
              show_column_names = F, heatmap_width= unit(20, "cm"), heatmap_height= unit(20, "cm"), col = heat_cols,
              top_annotation = hc1, border=T,
              column_split = factor(chr_sub$Chr, levels= unique(chr_sub$Chr)), cluster_column_slices = FALSE, column_gap = unit(0, "mm"),
              row_split = factor(annot_df$Celltype, levels=celltype_level), cluster_row_slices = FALSE,
              column_title_gp = gpar(fontsize=fontsize), row_title = NULL)

    #ht_list <- ha + hr1 + hr2 + hr3 + hr4
    ht_list <- ha + hr4
    #out for FEL013, too many cell?
    png(paste(out_loc, out_samp, "_", out_name, "_NoClust_Celltype.png",sep=""), width = 36, height = 36, units = "cm", res=300)
    ht <- draw(ht_list, ht_gap = unit(1, "mm"))
    dev.off()
}#end of if
...
if ( as.numeric(k_num) > 0 ) {
    ...
    ht_list <- ha + hr1 + hr2 + hr3 + hr4 + hr5
    ##out for FEL013, too many cell?
    png(paste(out_loc, out_samp, "_", out_name, "_k", k, ".png",sep=""), width = 36, height = 12, units = "cm", res=300)
    ht = draw(ht_list, ht_gap = unit(1, "mm"))
    dev.off()
    ...
    fwrite(annot_df, paste(out_samp, "_", out_name, ".clust", k, ".anno.txt", sep = ""), sep="\t", quote = F)
