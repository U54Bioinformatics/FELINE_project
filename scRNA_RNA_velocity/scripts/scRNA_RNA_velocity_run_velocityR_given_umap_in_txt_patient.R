...
obj_velocity=paste0(sample, "_", patient, "_velocity.rds")
if (!file.exists(obj_velocity)){
    ...
    # use only one patient, filter before overlapping umap
    # so we will have only cells from this patient to overlap with
    bm <- subset(bm, subset = orig.ident == patient)
    ...
    pdf(paste0(sample, "_", patient,  "_velocity_denovo_umap.pdf"))
    ...
    show.velocity.on.embedding.cor(emb = Embeddings(object = bm, reduction = "umap"), vel = Tool(object = bm,
       ...
       cex = 0.8, arrow.scale = 3, show.grid.flow = FALSE, min.grid.cell.mass = 0.5, grid.n = 40, arrow.lwd = 1,
...
pdf(paste0(sample, "_", patient, "_velocity_given_umap.pdf"))
...
write.table(vel_out$cc, paste0(sample, "_", patient, '_velocity.cc.txt'))
write.table(vel_out$tp, paste0(sample, "_", patient, '_velocity.tp.txt'))
write.table(vel_out$garrows, paste0(sample, "_", patient, '_velocity.garrows.txt'))
write.table(vel_out$arrows, paste0(sample, "_", patient, '_velocity.arrows.txt'))
write.table(vel_out$nd, paste0(sample, "_", patient, '_velocity.nd.txt'))
write.table(vel_out$es, paste0(sample, "_", patient, '_velocity.es.txt'))
write.table(vel_out$gv, paste0(sample, "_", patient, '_velocity.gv.txt'))
write.table(vel_out$gs, paste0(sample, "_", patient, '_velocity.gs.txt'))
write.table(vel_out$scale, paste0(sample, "_", patient, '_velocity.scale.txt'))
