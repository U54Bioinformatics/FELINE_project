...
if (!file.exists(obj_velocity)){
    ...
    write.table(cell.colors, paste0(sample, "_cell.colors.txt"), sep="\t", row.names=T, col.names=F, quote=F)
...
#ident.colors <- (scales::hue_pal())(n = length(x = levels(x = bm)))
#names(x = ident.colors) <- levels(x = bm)
#cell.colors <- ident.colors[bm$orig.ident]
#names(x = cell.colors) <- colnames(x = bm)
...
show.velocity.on.embedding.cor(emb = umap, vel = Tool(object = bm, slot = "RunVelocity"),
    n = 200, scale = "sqrt", cell.colors = ac(x = cell.colors, alpha = 0.5),
    cex = 0.8, arrow.scale = 3, show.grid.flow = TRUE, min.grid.cell.mass = 1, grid.n = 40, arrow.lwd = 1,
    do.par = FALSE, cell.border.alpha = 0.1)

legend("topleft", legend = unique(bm$orig.ident),
       col = unique(cell.colors), pch = 16, box.lwd = 0)
label_clusters(bm$orig.ident, umap, font = 2, cex = 1.2)

# transition probability matrix ($tp) and the velocity-(expression delta) correlation matrix ($cc)
# garrows:field arrow; arrows: cell arrow;
# for other files (nd, es, gv, gs, scale) read the funtion show.velocity.on.embedding.cor:
# https://github.com/JinfengChen/velocyto.R/blob/master/R/momentum_routines.R, show.velocity.on.embedding.cor 
