...
filter <- rowSums(assay(sce)>1)>3 #table(filter)
table(filter)
sce <- sce[filter,]
...
ggplot(ddFin_raw,aes(Dim1, Dim2,  col=CDC20)) + geom_point(size=4) + # , shape=coverage
  theme_classic()+ facet_wrap(~sample)

ggplot(ddFin,aes(Dim1, Dim2,  col=CDC20)) + geom_point(size=4) + # , shape=coverage
  theme_classic()+ facet_wrap(~sample)
dev.off()
