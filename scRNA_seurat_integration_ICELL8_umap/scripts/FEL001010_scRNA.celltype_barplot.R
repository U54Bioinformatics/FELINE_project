...
barplot_png <- function(filename, title, df, x_lab, y_lab, width=10, height=15){
    ...
    font_size = 14
    x_level = c("Cancer cells", "Normal epithelial cells", "Stromal cells", "Immune cells", "Unclassified")
...
x = fread("FEL001010.cell_metadata.txt")
x %>% filter(Platform %in% "ICELL8") -> x
...
filename <- "FEL001010_scRNA.celltype.barplot.pdf"
barplot_png(filename, "", data_df, "Patient", "Proportion", width=5, height=4)
