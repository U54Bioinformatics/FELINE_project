...
#SBATCH --mem=200G
#SBATCH --time=9:00:00
#SBATCH --output=Normalize_count_by_zinbwave.02_normalization.sh.%A_%a.stdout
#SBATCH -p all,abild
#SBATCH --workdir=./
...
echo "Split files"
#count=`cat file_B_cells.txt | head -n $N | tail -n 1`
...
#count=`cat file_Cancer_cells.txt | head -n $N | tail -n 1`
#count=`cat file_Normal_epithelial_cells.txt | head -n $N | tail -n 1`
