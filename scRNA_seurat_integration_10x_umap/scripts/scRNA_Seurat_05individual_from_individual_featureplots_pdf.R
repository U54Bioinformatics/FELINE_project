...
if (TRUE){
    ...
    c1 <- data.frame(
            ...
            Color = c("#FF9999", "#F1AB00", "#CAB388", "#B2C891", "#289728", "#6CABE7", "#E7298A", "#296DC0", "#E86A10", "#66A61E", "gray")
    )
    c <- data.frame(
            Cell.Type = c("Cancer cells", "Adipocytes", "B cells", "T cells", "Endothelial cells", "Normal epithelial cells", "Fibroblasts", "Macrophages", "Plasma cells", "Pericytes", "Low-quality cells"),
    ...
    filename1 <- paste(figdir, '/', fig_tsne, '.ggplot.plot.pdf', sep="")
    pdf(filename1, width=8, height=7)
    p
    dev.off()

    font_size = 66
    p <- ggplot(x, aes(x= tSNE_1, y= tSNE_2)) + geom_point(aes(fill=factor(Cell.Type)), size=10, shape=21, stroke=0) + scale_fill_manual(values=color_flag, labels=color_label) + labs(fill="", colour="", stroke=0) + theme(plot.title=element_text(size=font_size,face="bold"), axis.text=element_text(size=font_size, face="bold"), axis.title=element_text(size=font_size,face="bold"), legend.text=element_text(size=font_size, face="bold"), legend.title=element_text(size=font_size, face="bold")) + theme(panel.grid.major = element_blank(), panel.background = element_blank(), axis.line = element_line(colour = "black"), plot.margin = margin(1, 1, 1, 1, "cm")) + guides(shape = guide_legend(override.aes = list(size = 12)))
    filename2 <- paste(figdir, '/', fig_tsne, '.ggplot.png', sep="")
