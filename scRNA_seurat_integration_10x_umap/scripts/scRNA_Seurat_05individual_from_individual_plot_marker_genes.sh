...
#SBATCH --mem=280G
#SBATCH --time=9:00:00
#SBATCH --output=scRNA_Seurat_05individual_from_individual_plot_marker_genes.sh.%A_%a.stdout
#SBATCH -p abild,all
#SBATCH --workdir=./
...
sample=FEL011046_integrated_RPCA
#sample=FEL011043_integrated_standard
#sample=FEL011043_integrated_SCT
#sample=FEL011043_integrated_RPCA_default
platform=10x

if [ ! -e $sample\_$platform\_Seurat_marker_genes ]; then
    echo "plot marker genes: epithelial, immune and stromal"
    mkdir $sample\_$platform\_Seurat_marker_genes
    $R scRNA_Seurat_05individual_from_individual_plot_marker_genes.R $sample $platform
fi
