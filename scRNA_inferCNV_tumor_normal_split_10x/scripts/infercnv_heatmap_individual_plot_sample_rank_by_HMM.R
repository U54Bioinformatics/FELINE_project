...
#Annotation file
annot_df <- fread(paste(out_samp, "_HMM_infer.clust5.anno.txt", sep=""), header=T)
...
if ( as.numeric(noclust) > 0 ) {
    ...
    ha <- Heatmap(CNA_infer_M, cluster_rows = F, cluster_columns = F, show_row_names = T, show_heatmap_legend = FALSE,
              ...
              row_split = factor(annot_df$HMM_inferk5, levels= sort(unique(annot_df$HMM_inferk5))), cluster_row_slices = FALSE, 
              column_title_gp = gpar(fontsize=8), row_title = NULL)

    ht_list <- ha + hr1 + hr2 + hr3 + hr4 + hr5
    #out for FEL013, too many cell?
    png(paste(out_loc, out_samp, "_", out_name, "_Clust_by_HMM.png",sep=""), width = 36, height = 12, units = "cm", res=300)
