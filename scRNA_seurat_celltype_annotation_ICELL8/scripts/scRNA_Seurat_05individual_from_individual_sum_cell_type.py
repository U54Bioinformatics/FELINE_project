...
def main():
    ...
    parser.add_argument('--sample_cluster')
    parser.add_argument('-v', dest='verbose', action='store_true')
    args = parser.parse_args()
    try:
        len(args.cluster_anno) > 0 and len(args.sample_cluster) > 0
    except:
        usage()
        sys.exit(2)

    cluster_anno_file = re.sub(r'.txt', r'.cluster_anno.txt', args.cluster_anno)
    if not os.path.exists(cluster_anno_file):
        cluster_anno_file = read_table_Blueprint(args.cluster_anno)
    #os.system('sleep 30')
