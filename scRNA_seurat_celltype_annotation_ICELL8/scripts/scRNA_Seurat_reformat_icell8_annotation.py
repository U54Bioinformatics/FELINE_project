...
def read_cell_anno(infile):
    ...
    file_df = pd.read_table(infile, header=0)
    for i in range(file_df.shape[0]):
        data[file_df['Cell.ID'][i]] = [file_df['RNA_snn_res.0.8'][i], file_df['Encode_main_cluster_revised'][i]]
    return data

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('--meta')
    parser.add_argument('--cell_anno')
    parser.add_argument('-v', dest='verbose', action='store_true')
    args = parser.parse_args()
    try:
        len(args.cell_anno) > 0 and len(args.meta) > 0
    except:
        usage()
        sys.exit(2)


    cell_anno = read_cell_anno(args.cell_anno)
    read_meta_with_cluster(args.meta, cell_anno) 
