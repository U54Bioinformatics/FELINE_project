...
saveloc <- "~/Dropbox/Cancer_pheno_evo/data/FELINE2/CommunicationOutputAllArms/"

## Select a patient index to load cpm data
for(i in 1:length(CPMfiles)){ cat("patient index    ");cat(i);cat("    loading data    ") 
  ...
  for(tau in c(0,14,180)){    cat("    time    ");cat(tau);cat("   ");    
    ...
    if(nrow(phenotypes_i) > 0){
      ...
      grps <- c("gene", "samplesize_it", "Celltype", "PhenoCelltype", "key_", paste0("Disc_V", 1:5))  
...
## saved signalling sparse matrix in long format 
# recover using this approach:  
#(output%>%dplyr::select(key_,key_signaller, Signal)%>%spread(key_signaller, Signal))


#plot( Rmat[1,], RmatperSignaller[1,] )
#plot( Rmat[1,], RmatperSignaller[1,] * LRpair_it_dd$FracSample )
#ggplot(LRpair_it_dd[Pair.Name=="EGF_EGFR"], aes(y=TransductionperSignaller-Receptor,x=countofvalues,col=Pair.Name) ) + geom_point()+facet_wrap(~Celltype)
#ggplot(Communication1, aes(y=Transduction,x=Disc_V1,col=Pair.Name) ) + geom_point()+facet_grid(Pair.Name~Celltype)
#ggplot(LRpair_it_dd[Pair.Name=="EGF_EGFR"], aes(y=Transduction,x=Disc_V1,col=Pair.Name) ) + geom_point()+facet_wrap(~Celltype)
#ggplot(LRpair_it_dd[Pair.Name=="EGF_EGFR"], aes(y=(1+Transduction),x=(Ligand_N),col=PhenoCelltype) ) + geom_point()+facet_wrap(~PhenoCelltype)
#ggplot(LRpair_it_dd[Pair.Name=="EGF_EGFR"], aes(y=TransductionperSignaller,x=Disc_V1,col=Pair.Name) ) + geom_point()+facet_wrap(~Celltype)
#ggplot(LRpair_it_dd[Pair.Name=="EGF_EGFR"], aes(y=(1+TransductionperSignaller),x=(Ligand_N),col=PhenoCelltype) ) + geom_point()+facet_wrap(~PhenoCelltype)
#ggplot(LRpair_it_dd[Pair.Name=="EGF_EGFR"], aes(y=(1+TransductionperSignaller),x=(1+Transduction),col=PhenoCelltype) ) + geom_point()+facet_wrap(~PhenoCelltype)
