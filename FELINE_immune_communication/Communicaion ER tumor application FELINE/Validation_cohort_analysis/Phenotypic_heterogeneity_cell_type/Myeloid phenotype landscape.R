...
cell_types_all <-c("Macrophages");   #### CHANGE HERE compdataLU
...
ggplot(u_dat[abs(V1)<10],aes(V1,V2,col=Celltype_subtype))+theme_classic()+geom_point(size=0.61)+facet_wrap(~Quality)
ggplot(u_dat[abs(V1)<10&abs(V3)<10],aes(V2,V3,col=Celltype_subtype))+theme_classic()+geom_point(size=0.61)+facet_wrap(~Quality)
ggplot(u_dat[abs(V1)<10&abs(V3)<10],aes(V1,V3,col=Celltype_subtype))+theme_classic()+geom_point(size=0.61)+facet_grid(Celltype_subtype~Quality)

ggplot(u_dat[ARM!="A"][abs(V1)<2 & abs(V2)<2 & abs(V3)<2], aes(y=GSE13411_NAIVE_BCELL_VS_PLASMA_CELL_UP,x=Quality,group=Quality))+theme_classic()+geom_violin()+geom_point(size=1.5,alpha=0.5)+facet_grid(~Annotation2)


ggplot(u_dat[][abs(V1)<2 & abs(V2)<2 & abs(V3)<2],aes(V3,V2,col=Quality))+theme_classic()+geom_point(size=1.5,alpha=0.5)+facet_grid(~Annotation2)
ggplot(u_dat[][abs(V1)<2 & abs(V2)<2 & abs(V3)<2], aes(V3, V5,col= Annotation2) ) + theme_classic() + geom_point(size=1.5,alpha=0.2) + facet_grid(~Annotation2)


ggplot(u_dat[Quality=="HQ"][],aes(V2,V3,col=Annotation2))+theme_classic()+geom_point(size=1.5)+facet_grid(~Annotation2)
ggplot(u_dat[abs(V3)<4],aes(V3,V2,col=Annotation2))+theme_classic()+geom_point(size=1.5)+facet_grid(~Annotation2)
ggplot(u_dat[abs(V3)<4],aes(V5, V2, col=Quality))+theme_classic()+geom_point(size=1.5)+facet_grid(~Annotation2)
ggplot(u_dat[abs(V3)<4],aes(V5, V2, col=log(nCount_RNA)))+theme_classic()+geom_point(size=1.5)+facet_grid(~Annotation2)

names(u_dat) [grepl("REACTOME", names(u_dat))]
ggplot(u_dat[][abs(V1)<2],aes(V1,V2,col=log(nCount_RNA)))+theme_classic()+geom_point(size=1.5)
ggplot(u_dat[][abs(V1)<2],aes(V1,V2,col=Quality))+theme_classic()+geom_point(size=1.5)

cor(u_dat[ARM!="A"]$ REACTOME_EXTRACELLULAR_MATRIX_ORGANIZATION , u_dat[ARM!="A"] %>%select(V1,V2,V3,V4,V5))
cordd<-data.table(t(cor(u_dat %>%dplyr::select(V1,V2,V3,V4,V5) , u_dat%>%dplyr::select(common_ssgsea) )),keep.rownames = T)
cordd[order(-abs(V1))]
cordd[order(-abs(V2))]
cordd[order(-abs(V3))]
cordd[order(-abs(V4))]
...
data.table(t(cor(u_dat %>%dplyr::select(V1,V2,V3,V4,V5) , u_dat%>%dplyr::select(nCount_RNA) )),keep.rownames = T)
