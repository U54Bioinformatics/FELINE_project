...
cell_types_all <-c("Normal_epithelial_cells");   #### CHANGE HERE compdataLU
...
ggplot(u_dat[abs(V1)<10],aes(V1,V3,col=Annotation2))+theme_classic()+geom_point(size=0.61)+facet_wrap(~Quality)

ggplot(u_dat[abs(V1)<10],aes(V1,V3,col=Annotation2))+theme_classic()+geom_point(size=0.61)
...
ggplot(u_dat[ARM!="A"][abs(V1)<2],aes(V1,V5,col=Quality))+theme_classic()+geom_point(size=1.5,alpha=0.5)+facet_grid(Day~dynamic_class3)
