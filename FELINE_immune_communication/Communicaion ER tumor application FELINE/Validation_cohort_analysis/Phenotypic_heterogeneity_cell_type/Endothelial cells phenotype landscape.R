...
cell_types_all <-c("Endothelial_cells");   #### CHANGE HERE compdataLU
...
u_datsubset <- merge(metadd[Day %in% DAY][ARM %in% ARMS][file_string%in%cell_types_all],
               data.table(Cell.ID=rownames(umap_data_tt$layout), umap_data_tt$layout )
               ,by="Cell.ID") 
...
ggplot(u_dat,aes(V1,V2,col=Celltype_subtype))+theme_classic()+geom_point(alpha=0.1)+facet_wrap(~Quality)
ggplot(u_dat,aes(V1,V2,col=Quality))+theme_classic()+geom_point(alpha=0.1)+facet_wrap(~Celltype_subtype)
ggplot(u_dat,aes(V1,V3,col=Celltype_subtype))+theme_classic()+geom_point(size=.51)
ggplot(u_dat,aes(V1,V3,col=PETROVA_ENDOTHELIUM_LYMPHATIC_VS_BLOOD_UP))+theme_classic()+geom_point(size=.51)

ggplot(u_dat,aes(PETROVA_ENDOTHELIUM_LYMPHATIC_VS_BLOOD_UP,Celltype_subtype))+theme_classic()+geom_violin(aes(fill=Quality,group=interaction(Celltype_subtype, Quality)))
#ggplot(u_dat,aes(REACTOME_EXTRACELLULAR_MATRIX_ORGANIZATION,Celltype_subtype))+theme_classic()+geom_violin(aes(fill=Quality,group=interaction(Celltype_subtype, Quality)))
ggplot(u_dat,aes(PID_ENDOTHELIN_PATHWAY,Celltype_subtype))+theme_classic()+geom_violin(aes(fill=Quality,group=interaction(Celltype_subtype, Quality)))
ggplot(u_dat,aes(PID_LYMPH_ANGIOGENESIS_PATHWAY,Celltype_subtype))+theme_classic()+geom_violin(aes(fill=Quality,group=interaction(Celltype_subtype, Quality)))

ggplot(u_dat,aes(PETROVA_ENDOTHELIUM_LYMPHATIC_VS_BLOOD_UP,log(1+nCount_RNA))) +theme_classic()+geom_point(aes(col=Quality))+facet_wrap(~Celltype_subtype)
ggplot(u_dat,aes(PETROVA_ENDOTHELIUM_LYMPHATIC_VS_BLOOD_UP,log(1+nCount_RNA),shape=Celltype_subtype )) +theme_classic()+geom_point(aes(col=Quality))#+facet_wrap(~Celltype_subtype)
ggplot(u_dat,aes(PETROVA_ENDOTHELIUM_LYMPHATIC_VS_BLOOD_UP,log(1+nCount_RNA),shape=Quality )) +theme_classic()+geom_point(aes(col=Celltype_subtype))#+facet_wrap(~Celltype_subtype)
m1<-lm(PETROVA_ENDOTHELIUM_LYMPHATIC_VS_BLOOD_UP~log(1+nCount_RNA),u_dat)
dat2 <- data.table(u_dat %>% select(PETROVA_ENDOTHELIUM_LYMPHATIC_VS_BLOOD_UP,nCount_RNA,Quality,Celltype_subtype ), resid = m1$residuals )

ggplot(dat2,aes(resid,Celltype_subtype))+geom_violin(aes(fill=Quality, group=interaction(Celltype_subtype,Quality)))
ggplot(dat2,aes(PETROVA_ENDOTHELIUM_LYMPHATIC_VS_BLOOD_UP,Celltype_subtype))+geom_violin(aes(fill=Quality, group=interaction(Celltype_subtype,Quality)))
ggplot(u_dat,aes(PETROVA_ENDOTHELIUM_LYMPHATIC_VS_BLOOD_DN,Celltype_subtype))+geom_violin(aes(fill=Quality, group=interaction(Celltype_subtype,Quality)))

names(u_dat) [grepl("REACTOME", names(u_dat))]
ggplot(u_dat,aes(V1,V2,col=PETROVA_ENDOTHELIUM_LYMPHATIC_VS_BLOOD_UP))+theme_classic()+geom_point(size=1.5,alpha=0.5)+facet_grid(Celltype_subtype~Quality)
ggplot(u_dat,aes(V1,V2,col=Quality))+theme_classic()+geom_point(size=1.5,alpha=0.5)+facet_grid(~Celltype_subtype)
ggplot(u_dat,aes(V1,V2,col=Celltype_subtype))+theme_classic()+geom_point(size=1.5,alpha=0.5)+facet_grid(~Quality)

cor(u_dat[ARM!="A"]$ REACTOME_EXTRACELLULAR_MATRIX_ORGANIZATION , u_dat[ARM!="A"] %>%select(V1,V2,V3,V4,V5))
cordd<-data.table(t(cor(u_dat %>%dplyr::select(V1,V2,V3,V4) , u_dat%>%dplyr::select(common_ssgsea) )),keep.rownames = T)
cordd[order(-abs(V1))]
cordd[order(-abs(V2))]
cordd[order(-abs(V3))]
cordd[order(-abs(V4))]
