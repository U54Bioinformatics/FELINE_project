...
cell_types_all <-c("Cancer_cells");   #### CHANGE HERE compdataLU
...
downsam.metadd <- data.table( metadd[ Day %in% DAY][ ARM %in% ARMS ][file_string%in%cell_types_all][ order( -nCount_RNA ) ] %>% 
...
NN <- 4  ### CHANGE TO MATCH Cohort 1
set.seed(123); umap_data_tt <- umap(ss_dd, n_components=NN, n_neighbors=5)     #cor(umap_data_tt$layout) #pairs(umap_data_tt$layout)

# Reshape all ssgsea data (train and out of sample cells)
fullcel_gen_sub <- SSGSEA [Gene_Set %in% colnames(ss_dd) ,][, names(SSGSEA) %in% c("Gene_Set", metadd[Day %in% DAY][ARM %in% ARMS][file_string%in%cell_types_all]$Cell.ID ), with=FALSE ] 
rm(list="ss_dd")
...
all_1 <- SSGSEA [, names(SSGSEA) %in% c("Gene_Set", metadd[Day %in% DAY][ARM %in% ARMS][file_string%in%cell_types_all]$Cell.ID), with=FALSE ]
...
if(nrow(umap_data_tt$layout) == nrow(metadd[Day %in% DAY][ARM %in% ARMS][file_string%in%cell_types_all]) ){
  u_dat <- merge(metadd[Day %in% DAY][ARM %in% ARMS][file_string%in%cell_types_all],
...
# need:
#u_dat
Subtype <- unique(metadd[file_string%in%cell_types_all]$Celltype_subtype)
save(SSGSEA,DAY,cell_types_all,ARMS,common_ssgsea,n_ssgsea,Subtype, file=paste0("~/Dropbox/Cancer_pheno_evo/data/FELINE2/PhenotypesAllArmsCohort2/UpdateRevisednewIntermediateTMP2.RData"))
save(umap_data_tt, file=paste0("~/Dropbox/Cancer_pheno_evo/data/FELINE2/PhenotypesAllArmsCohort2/UpdateRevisednewIntermediateTMP5.RData"))
save(full_dd_transp ,  file=paste0("~/Dropbox/Cancer_pheno_evo/data/FELINE2/PhenotypesAllArmsCohort2/UpdateRevisednewIntermediateTMP.RData"))
save( allgene_dd_transp, file=paste0("~/Dropbox/Cancer_pheno_evo/data/FELINE2/PhenotypesAllArmsCohort2/UpdateRevisednewIntermediateTMP3.RData"))
rm(list=c("SSGSEA","fullcel_gen_sub"))
rm(list="allgene_dd_transp")
u_datReduced <-  data.table(Cell.ID=rownames(full_dd_transp), predict(umap_data_tt,data= full_dd_transp  ))
save(u_datReduced, file=paste0("~/Dropbox/Cancer_pheno_evo/data/FELINE2/PhenotypesAllArmsCohort2/RevisednewIntermediateTMP4.RData"))

u_dat <- merge(metadd[Day %in% DAY][ARM %in% ARMS][file_string %in% cell_types_all],
               merge(u_datReduced, setnames(data.table(full_dd_transp,keep.rownames = T),old="rn", new="Cell.ID"),by="Cell.ID")  ,by="Cell.ID") 
rm(list="full_dd_transp")
save(u_dat, file=paste0("~/Dropbox/Cancer_pheno_evo/data/FELINE2/PhenotypesAllArmsCohort2/UpdateRevisednewIntermediateTMP6.RData"))

rm(list="full_dd_transp","umap_data_tt")
load(  file=paste0("~/Dropbox/Cancer_pheno_evo/data/FELINE2/PhenotypesAllArmsCohort2/RevisednewIntermediateTMP3.RData"))
load(  file=paste0("~/Dropbox/Cancer_pheno_evo/data/FELINE2/PhenotypesAllArmsCohort2/RevisednewIntermediateTMP2.RData"))

u_dat <- merge(metadd[Day %in% DAY][ARM %in% ARMS][file_string %in% cell_types_all],
               data.table(u_datReduced, allgene_dd_transp)  ,by="Cell.ID") 
load(  file=paste0("~/Dropbox/Cancer_pheno_evo/data/FELINE2/PhenotypesAllArmsCohort2/RevisednewIntermediateTMP5.RData"))
...
# 
ggplot(u_dat[ARM!="A"], aes(y=HALLMARK_INTERFERON_GAMMA_RESPONSE,x=log(1+Day), col=dynamic_class3, group=interaction(Day,dynamic_class3) ) ) + 
  theme_classic()+geom_point(position = position_dodge(width=1),     size=1.5,alpha=0.5)+geom_violin()

plot.dat<- u_dat%>%select(Day,ARM,dynamic_class3,HALLMARK_INTERFERON_GAMMA_RESPONSE)

plot.dat[,Treatment:="Combination ribociclib"]
plot.dat[ARM=="A",Treatment:="Letrozole alone"]

summary(lm(HALLMARK_INTERFERON_GAMMA_RESPONSE~dynamic_class3*Treatment , plot.dat[Day==180]))
ggplot(plot.dat[Day==180], aes(y=HALLMARK_INTERFERON_GAMMA_RESPONSE,x=dynamic_class3, col=dynamic_class3, group=interaction(dynamic_class3) ) ) + theme_classic()+
  theme_classic(base_size=18)+geom_point(position = position_dodge(width=1),     size=1.5,alpha=0.5)+
  geom_boxplot(aes(fill=dynamic_class3),alpha=0.6)+
  #geom_violin()+
  facet_wrap(~Treatment)+theme(aspect.ratio=1,legend.position = "none")+
  scale_x_discrete(labels=c("Resistant", "Sensitive"))+ scale_color_npg()+scale_fill_npg()+
  labs(y="Cancer interferon gamma response \n post treatment (Hallmark ssGSEA at day 180)", x="Tumor response")
...
#ggsave(paste0(paperfile,"Ribo and Letrozole Hallmark IFN gamma validation.png"),height=10,width=10)



# 
summary( lm(HALLMARK_INTERFERON_GAMMA_RESPONSE~dynamic_class3,   u_dat[ARM!="A"] )  )

ggplot(u_dat[ARM!="A"][abs(V2)<2][,1:50], aes(V2,V1, col=Quality ) ) + theme_classic()+geom_point(size=1.5,alpha=0.5)
