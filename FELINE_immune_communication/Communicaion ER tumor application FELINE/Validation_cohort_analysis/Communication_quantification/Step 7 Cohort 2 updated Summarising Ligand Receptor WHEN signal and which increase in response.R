...
allcomms<- rbind( initdiffs[ReceptorPhenoCelltype=="Macrophages"][LigandPhenoCelltype%in%"Cancer cells"]%>%dplyr::select(Pair.Name,LigandPhenoCelltype, ReceptorPhenoCelltype,Treat,adjust.p.val, )
,
assessment[Treat=="CombinationRibo"][Model=="one way anova_DayStart_Response"][ReceptorPhenoCelltype=="Macrophages"][LigandPhenoCelltype%in%"Cancer cells"][Pair.Name%in%c(
  "SERPINC1_LRP1",  "SERPINE2_LRP1","SEMA3B_NRP1", "SEMA3B_NRP2",#"SEMA4F_NRP2",
  #"FGF2_SDC2","FGF2_SDC3","COL5A3_SDC3","FGF2_FGFR1","FGF2_CD44",
  "MADCAM1_CD44" ) ]%>%dplyr::select(Pair.Name,LigandPhenoCelltype, ReceptorPhenoCelltype,Treat,adjust.p.val, )
)
...
summarCommVsGrowthC1C2<- data.table( CCIAll_CtoMpathsC1C2 [! Pair.Name%in%c("FGF2_SDC2","FGF2_SDC3","COL5A3_SDC3","FGF2_FGFR1","FGF2_CD44")] %>%  
...
ggplot(summarCommVsGrowthC1C2%>%group_by(Cohort)%>%mutate(Mu1= mean(scalelnTransductionMu1)),
       aes(y=scalelnTransductionMu1-Mu1, x=prop_change,ymax=ucl-Mu1,ymin=lcl-Mu1))+ theme_classic()+
  geom_point(aes(col=dynamic_class3),size=4) +
  geom_errorbar()+
  labs(y="Pre-treatment cancer communication with macrophages",x="Fraction of tumor remaining")+
  ...
  scale_color_npg(name="Tumor response", labels=c("Resistant", "Sensitive"))+
  scale_fill_npg(name="Tumor response", labels=c("Resistant", "Sensitive")) +
  geom_smooth(method="gam",se=T )+facet_wrap(CommPredictor~Cohort,scale="free")
