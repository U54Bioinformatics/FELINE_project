...
#Annotation file
annot_df <- fread(paste(out_samp, "_cell_metadata.txt", sep=""), header=T)
names(annot_df) <- c("Cell.ID", "Sample", "nCount_RNA", "Total_Reads_k", "nFeature_RNA", "S", "G2M", "Phase", "percent.mt", "Cell_type", "seurat_clusters")
...
print("check point 3")
####No Clustering
if ( as.numeric(noclust) > 0 && !file.exists(paste(out_loc, out_samp, "_", out_name, "_NoClust_Sample.png",sep=""))) {
...
if ( as.numeric(noclust) > 0 && !file.exists(paste(out_loc, out_samp, "_", out_name, "_NoClust_seurat_clusters.png",sep=""))) {
    print(c("noclust", noclust))
    print('Plot nocluster plot ......')
    ha <- Heatmap(CNA_infer_M, cluster_rows = F, cluster_columns = F, show_row_names = T, show_heatmap_legend = FALSE,
...
if ( as.numeric(noclust) > 0 && !file.exists(paste(out_loc, out_samp, "_", out_name, "_NoClust_Encode_main_cluster.png",sep=""))) {
    print(c("noclust", noclust))
    print('Plot nocluster plot ......')
    ha <- Heatmap(CNA_infer_M, cluster_rows = F, cluster_columns = F, show_row_names = T, show_heatmap_legend = FALSE,
              show_column_names = F, heatmap_width= unit(20, "cm"), heatmap_height= unit(60, "cm"), col = heat_cols,
...
if ( as.numeric(k_num) > 0 && !file.exists(paste(out_loc, out_samp, "_", out_name, "_k", k_num, ".png",sep=""))) {
    #print("check point 5")
