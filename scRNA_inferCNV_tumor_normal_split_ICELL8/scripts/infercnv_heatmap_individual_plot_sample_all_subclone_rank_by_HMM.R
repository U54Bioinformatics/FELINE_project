...
#Annotation file
annot_df <- fread(paste(out_samp, "_", "HMM_infer.subclone", k_num, ".anno.txt", sep = ""), header=T)
names(annot_df) <- c("Cell.ID", "Sample", "nCount_RNA", "Total_Reads_k", "nFeature_RNA", "S", "G2M", "Phase", "percent.mt", "Cell_type", "seurat_clusters", "tumor_normal", "HMM_inferk")
...
#order by cells in cluster order
#counts_sub <- counts_sub[annot_df$Cell.ID,]
...
if ( as.numeric(noclust) > 0 ) {
    ...
    png(paste(out_loc, out_samp, "_", out_name, ".subclone_k", k_num ,".png",sep=""), width = 36, height = 12, units = "cm", res=300)
